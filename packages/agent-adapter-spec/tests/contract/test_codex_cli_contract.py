...
def _fake_codex(bin_dir):
    ...
    command.write_text(
        ...
        "  fail) printf '%s\\n' '{\"type\":\"turn.failed\",\"error\":{\"message\":\"boom\"}}'; exit 7 ;;\n"
        "  *) printf '%s\\n' '{\"type\":\"item.started\",\"item\":{\"id\":\"item_0\",\"type\":\"command_execution\",\"command\":\"echo hello\",\"aggregated_output\":\"\",\"exit_code\":null,\"status\":\"in_progress\"}}'; printf '%s\\n' '{\"type\":\"item.completed\",\"item\":{\"id\":\"item_1\",\"type\":\"agent_message\",\"text\":\"hello-from-codex\"}}' ;;\n"
        "esac\n"
    )
    command.chmod(0o755)
...
def _paths(tmp_path):
    ...
    _fake_codex(bin_dir); _plant_credentials(provider_home)
...
async def test_env_values_never_appear_in_emitted_events(tmp_path):
    ...
    adapter = CodexCliAdapter()
    await adapter.start(_request(workspace, provider_home, bin_dir, env={"OPENAI_API_KEY": secret}))
    events = await _events(adapter)
