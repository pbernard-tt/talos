...
"""Section 7.2 contract suite against ClaudeCodeAdapter using a deterministic fake CLI."""
...
def _fake_claude(bin_dir):
    ...
    command.write_text(
        ...
        "  fail) printf '%s\\n' '{\"type\":\"result\",\"subtype\":\"error\",\"result\":\"failed\"}'; exit 7 ;;\n"
        "  *) printf '%s\\n' '{\"type\":\"assistant\",\"message\":{\"content\":[{\"type\":\"tool_use\",\"name\":\"Bash\",\"input\":{\"command\":\"echo hello\"}},{\"type\":\"text\",\"text\":\"hello-from-claude\"}]}}'; printf '%s\\n' '{\"type\":\"result\",\"subtype\":\"success\",\"result\":\"done\"}' ;;\n"
        "esac\n"
    )
    command.chmod(0o755)
...
def _paths(tmp_path):
    ...
    workspace.mkdir(); provider_home.mkdir(); bin_dir.mkdir(); _fake_claude(bin_dir)
...
async def test_writes_nothing_outside_workspace_and_provider_home(tmp_path):
    ...
    assert (provider_home / ".claude" / "settings.json").is_file()
...
async def test_env_values_never_appear_in_emitted_events(tmp_path):
    ...
    adapter = ClaudeCodeAdapter()
    await adapter.start(_request(workspace, provider_home, bin_dir, env={"ANTHROPIC_API_KEY": secret}))
    events = await _events(adapter)
