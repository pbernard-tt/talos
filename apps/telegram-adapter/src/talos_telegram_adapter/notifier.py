...
the same pattern apps/orchestrator uses for its own queues. Chat receives notifications with dashboard
deep links only; approval *decisions* stay dashboard-only (Section 16 Phase 12 Track B)."""
...
DLQ_QUEUE = "talos.dlq"
NOTIFIER_QUEUE = "talos.notifiers.telegram"
...
async def start_consuming(
    ...
    await queue.consume(
        functools.partial(_handle, redis_client=redis_client, telegram_client=telegram_client, settings=settings)
    )
    logger.info("talos-telegram-adapter consuming %s", NOTIFIER_QUEUE)
