...
async def test_create_task_resolves_project_and_creates_task():
    ...
    reply = await handle_command(
        api_client,
        {"channel": "TELEGRAM", "chat_id": "42", "command": "CREATE_TASK", "project_name": "Talos", "title": "Fix bug"},
    )
...
async def test_create_task_unknown_project_does_not_call_create_and_replies_clearly():
    api_client = FakeApiClient()

    reply = await handle_command(
        api_client,
        {"channel": "TELEGRAM", "chat_id": "42", "command": "CREATE_TASK", "project_name": "Nope", "title": "Fix bug"},
    )
...
async def test_task_status_reports_status():
    ...
    reply = await handle_command(api_client, {"channel": "TELEGRAM", "chat_id": "42", "command": "TASK_STATUS", "task_id": "task-1"})
...
async def test_task_status_unknown_task_reports_error_without_raising():
    ...
    reply = await handle_command(api_client, {"channel": "TELEGRAM", "chat_id": "42", "command": "TASK_STATUS", "task_id": "missing"})
...
async def test_run_status_reports_status():
    ...
    reply = await handle_command(api_client, {"channel": "TELEGRAM", "chat_id": "42", "command": "RUN_STATUS", "run_id": "run-1"})
...
async def test_list_approvals_reports_none_pending():
    ...
    reply = await handle_command(api_client, {"channel": "TELEGRAM", "chat_id": "42", "command": "LIST_APPROVALS"})
...
async def test_list_approvals_lists_each_pending_approval():
    api_client = FakeApiClient()
    api_client.pending_approvals = [
        {"id": "appr-1", "runId": "run-1", "approvalType": "PUSH"},
        {"id": "appr-2", "runId": "run-2", "approvalType": "DEPLOY"},
    ]

    reply = await handle_command(api_client, {"channel": "TELEGRAM", "chat_id": "42", "command": "LIST_APPROVALS"})

    assert "appr-1" in reply
    assert "appr-2" in reply
