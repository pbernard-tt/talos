...
## 6. Runner supervisor's own auth boundary (noted, not closed, this phase)

`talos-runner-supervisor`'s HTTP endpoints (`/workspaces/prepare`, `/runs/{id}/execute`, `/workspaces/cleanup`, etc.) do not themselves check `TALOS_INTERNAL_API_TOKEN` -- the setting exists (`Settings.internal_api_token`) but nothing in `app.py` enforces it as a request-level auth check. The security boundary today is purely network topology: only `talos-orchestrator` is meant to be able to reach `talos-runner-supervisor` on the internal Docker network, and (per Section 2's design note above) per-run containers are deliberately kept off that path by never attaching the supervisor to `talos_run_network`. Adding real request-level auth to the runner supervisor's own endpoints (matching the pattern already used for `talos-api`'s `/internal/v1/**`) is flagged as a follow-up, not fixed in this pass, since it isn't one of Phase 11's stated acceptance criteria and the network-topology boundary already holds given the isolation work above.
